...
export default function ContainersList() {
  ...
  const handleDelete = async (containerId: string, volume: number) => {
    if (confirm(`Are you sure you want to delete the ${volume}m³ container?`)) {
      try {
  ...
  return (
    <>
      {/* Add New Container Button - Moved to top */}
      <button 
      ...
      >
        ...
        <div className="text-left">
          <span className="text-sm font-semibold block">Add New Container</span>
          <span className="text-xs text-gray-500 group-hover:text-gray-400">Create a new container option</span>
        </div>
      </button>
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2" onClick={(e) => e.stopPropagation()}>
              <button
                ...
                title={container.isActive ? 'Hide from website' : 'Show on website'}
              >
                {container.isActive ? (
                  <>
