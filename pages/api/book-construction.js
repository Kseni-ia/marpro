...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('Received construction booking request:', {
    ...
    // Ensure service type is set to constructions
    customerData.serviceType = 'constructions';

    // Check availability first
    const isAvailable = await checkAvailability(dateTime, durationHours, 'constructions');
    ...
    try {
      ...
      console.log('Creating construction order with data:', {
      ...
      return res.status(500).json({ 
        error: 'Failed to create construction order',
    ...
    const emailParams = {
      ...
      constructionType: customerData.constructionType,
    ...
    res.status(200).json({
      success: true,
      message: 'Construction booking created successfully!',
      orderId: orderId,
    ...
    res.status(500).json({ 
      error: 'Failed to process construction booking',
      details: error.message,
